# -*- coding: utf-8 -*-
"""
LNP_DC2_4_publication_safe_nestedCV_v2_3_cumulative_rounds.py
...
Primary nested-CV analysis:
    python LNP_DC2_4_publication_safe_nestedCV_v2.py \
        --data-path "C:\\path\\All 100 DATA in same log value and HaCaT.xlsx"
...
SCRIPT_VERSION = "2.3-publication-safe-cumulative-rounds"
DEFAULT_FILE = "All 100 DATA in same log value and HaCaT.xlsx"
...
def find_existing_data_path(user_path: Optional[str]) -> str:
    ...
    candidates: List[str] = []
    for username in [os.path.basename(home), "Joey", "ASUS"]:
        for base in [
            rf"C:\Users\{username}\Desktop",
            rf"C:\Users\{username}\桌面",
            rf"C:\Users\{username}\OneDrive\Desktop",
            rf"C:\Users\{username}\OneDrive\桌面",
        ]:
            candidates.extend([
                os.path.join(base, "AI screen LNP python and excel", "PyCharm 有效代码", DEFAULT_FILE),
                os.path.join(base, DEFAULT_FILE),
            ])
    for candidate in candidates:
        if os.path.exists(candidate):
            return os.path.abspath(candidate)

    roots = [os.path.join(home, "Desktop"), os.path.join(home, "桌面"), home]
    if os.path.isdir(home):
        roots.extend(os.path.join(home, d) for d in os.listdir(home) if d.lower().startswith("onedrive"))
    for root in roots:
        if not os.path.isdir(root):
            continue
        try:
            hits = glob.glob(os.path.join(root, "**", DEFAULT_FILE), recursive=True)
        except Exception:
            hits = []
        if hits:
    ...
    raise FileNotFoundError(
        f"Could not find '{DEFAULT_FILE}'. Pass the workbook explicitly with --data-path."
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--data-path", default="", help="Training Excel workbook path.")
