...
same scale as the training target. The manuscript analysis uses the exact workbook column
"Normalized for DC2.4". For prospective validation, external experimental values must be
placed on the identical target scale before RMSE/R2 are interpreted quantitatively.

Typical commands
----------------
Primary nested-CV analysis:
    python DC24_R1R4_ModelSelection_Publication_GitHub.py \
...
SCRIPT_VERSION = "2.4.0-R1R4-publication-github"
...
# -----------------------------------------------------------------------------
# Publication target definition
# -----------------------------------------------------------------------------
# Direct-run convenience for the author's current workstation. This preserves
# the same workbook location used by the previously working script, so the file
# can be launched directly from Downloads/PyCharm without command-line arguments.
# For GitHub/reproducible use, --data-path still overrides this path and the
# repository-relative fallbacks below remain available.
...
DEFAULT_REPO_DATA_CANDIDATES = (
    "data/processed/lnp_dc24_r1_r4_normalized_1_35.xlsx",
    "data/processed/R1-4 all LNP normalized 1.35 (new).xlsx",
    "lnp_dc24_r1_r4_normalized_1_35.xlsx",
    "R1-4 all LNP normalized 1.35 (new).xlsx",
)
FORCED_SHEET_NAME = "Round1&2&3&4"
FORCED_NORMALIZED_TARGET = "Normalized for DC2.4"  # exact target used in the manuscript analysis
NORMALIZATION_VERSION = "1.35"
...
def _round_number(value: Any) -> Optional[int]:
    if value is None or (isinstance(value, float) and np.isnan(value)):
        return None
    match = re.search(r"R\s*(\d+)", str(value), re.I)
    return int(match.group(1)) if match else None


def infer_round_from_candidate_id(candidate_id: Any) -> str:
    """Infer the actual experimental round from candidate labels.

    Historical replicate labels can contain more than one round identifier,
    for example ``R1-17 (R3-28)``. The measurement belongs to the later
    experimental round, so the highest round number in the label is used.
    """
    if candidate_id is None or (isinstance(candidate_id, float) and np.isnan(candidate_id)):
        return "UNKNOWN"
    text = str(candidate_id).strip()
    round_numbers = [int(x) for x in re.findall(r"R\s*(\d+)", text, flags=re.I)]
    if round_numbers:
        return f"R{max(round_numbers)}"
...
def fill_missing_round_labels(df_in: pd.DataFrame) -> pd.DataFrame:
    """Resolve experimental-round labels without moving historical replicates
    back to an earlier round.

    If both an explicit Round column and a candidate-ID-derived round are
    available, the later (higher-numbered) round is retained.
    """
    out = df_in.copy()
    explicit = (
        out["Round"].apply(normalize_round_label)
    ...
    for exp, inf in zip(explicit, inferred):
        exp_n = _round_number(exp)
        inf_n = _round_number(inf)
        if exp_n is not None and inf_n is not None:
            resolved.append(f"R{max(exp_n, inf_n)}")
        elif inf_n is not None:
            resolved.append(f"R{inf_n}")
        elif exp_n is not None:
            resolved.append(f"R{exp_n}")
        else:
            resolved.append("UNKNOWN")

    out["Round"] = resolved
    out["Round_inferred_from_ID"] = inferred.values

    multi_round_mask = out["candidate_id"].astype(str).apply(
        lambda s: len(re.findall(r"R\s*(\d+)", s, flags=re.I)) > 1
    )
    if multi_round_mask.any():
        print("[RoundAudit] Multi-round replicate labels resolved using the highest round number:")
        for _, row in out.loc[multi_round_mask, ["candidate_id", "Round"]].iterrows():
            print(f"  {row['candidate_id']} -> {row['Round']}")

    counts = out["Round"].value_counts(dropna=False).to_dict()
    print(f"[Round] Labels after corrected recovery: {counts}")
    return out
...
def find_existing_data_path(user_path: Optional[str]) -> str:
    """Resolve the training workbook without machine-specific paths.

    Priority:
    1. explicit ``--data-path``;
    2. repository-relative/default candidate paths;
    3. accepted workbook names beside the script or in ``data/processed``.
    """
    def clean_path(value: Any) -> Path:
        return Path(os.path.expandvars(os.path.expanduser(str(value).strip().strip('"').strip("'"))))

    def is_excel_file(path: Path) -> bool:
        return path.is_file() and not path.name.startswith("~$") and path.suffix.lower() in {".xlsx", ".xlsm", ".xls"}

    script_dir = Path(__file__).resolve().parent
    repo_root = script_dir
    # Walk upward to a likely repository root.
    for parent in [script_dir] + list(script_dir.parents):
        if (parent / "README.md").exists() or (parent / ".git").exists():
            repo_root = parent
            break

    # 0. Direct-run mode: use exactly the same local workbook as the previously
    # working script when no --data-path is supplied.
    if not user_path and FORCED_DATA_PATH.is_file():
        resolved = FORCED_DATA_PATH.resolve()
        print(f"[DataPath] Using direct-run workbook: {resolved}")
        return str(resolved)

    if user_path:
        supplied = clean_path(user_path)
        candidates = [supplied] if supplied.is_absolute() else [Path.cwd() / supplied, repo_root / supplied, script_dir / supplied]
        for candidate in candidates:
        ...
        raise FileNotFoundError(
            "--data-path was provided, but the workbook was not found. Checked:\n"
            + "\n".join(f"  - {p}" for p in candidates)
        )

    candidates: List[Path] = []
    for rel in DEFAULT_REPO_DATA_CANDIDATES:
        candidates.extend([repo_root / rel, script_dir / rel])
    for filename in DATA_FILE_CANDIDATES:
        candidates.extend([script_dir / filename, repo_root / "data" / "processed" / filename])
    ...
    for candidate in candidates:
        ...
        if is_excel_file(candidate):
            resolved = candidate.resolve()
            print(f"[DataPath] Auto-detected workbook: {resolved}")
            return str(resolved)

    raise FileNotFoundError(
        "Could not find the R1-R4 training workbook. Pass it explicitly with --data-path.\n"
        "Example: python DC24_R1R4_ModelSelection_Publication.py --data-path data/processed/lnp_dc24_r1_r4_normalized_1_35.xlsx"
    )
...
def detect_candidate_id_column_by_values(df: pd.DataFrame) -> Optional[str]:
    """
    Safety fallback for raw combined workbooks where the ID column may be named
    'No', 0, or something unusual. Prefer the column containing R2-/R3-/R4-
    labels plus the numeric R1 identifiers.
    """
    best_col = None
    best_score = -1
...
def standardize_formulation_columns(df_in: pd.DataFrame, require_target: bool = True) -> Tuple[pd.DataFrame, Dict[str, str]]:
    ...
    # If the workbook uses an unusual header (for example 0) for the ID column,
    # detect it from the R2-/R3-/R4- values themselves.
    if "candidate_id" not in source_columns:
        detected_id = detect_candidate_id_column_by_values(df)
        if detected_id is not None:
            rename[detected_id] = "candidate_id"
            source_columns["candidate_id"] = detected_id
            print(f"[IDCheck] candidate_id auto-detected from values: {detected_id!r}")
...
def attach_training_target(
    ...
    if target_mode == "normalized":
        # This dedicated script uses ONE AND ONLY ONE target column:
        #     Normalized for DC2.4
        #
        # No fallback to:
        ...
        # - Li-Min
        # - same log value
        # - raw DC_Cell_Transfection_Efficiency
        # - any other normalized-looking column
        source = find_column(original_df, [FORCED_NORMALIZED_TARGET])

        if source is None:
            raise ValueError(
                f"找不到指定目标列 {FORCED_NORMALIZED_TARGET!r}。"
                f"\n当前工作表真实列名为：{list(original_df.columns)}"
            )

        # Report Excel column letter for manual verification.
        col_index = list(original_df.columns).index(source) + 1
        n = col_index
        letters = ""
        while n:
            n, rem = divmod(n - 1, 26)
        ...
        print("=" * 88)
        print("[TARGET LOCK]")
        print(f"[TargetCheck] Using EXACT target = {source!r}")
        print(f"[TargetCheck] Excel column letter = {letters}")
        print(f"[TargetCheck] Non-missing values = {int(values.notna().sum())}/{len(values)}")
        print("[TargetCheck] NO fallback target is allowed in this script.")
        print("=" * 88)
...
# -----------------------------------------------------------------------------
# Robust workbook sheet resolver
# -----------------------------------------------------------------------------
def _sheet_match_key(name: Any) -> str:
    """
    Normalize worksheet names for matching:
...
def _sheet_base_key(name: Any) -> str:
    """
    More relaxed key:
    also ignore a trailing '(2)', '(3)', etc.
    Example:
        Round1&2&3&4
        Round1&2&3&4 (2)
        Round1&2&3&4  (2)
    are treated as the same logical sheet family.
    """
...
def resolve_training_sheet_name(workbook_path: str, requested: Any) -> Any:
    ...
    4. if exactly one sheet belongs to the Round1&2&3&4 family, use it

    If still ambiguous, print every real sheet name with repr() and stop,
    instead of silently reading the wrong sheet.
    """
    # Numeric sheet index is still supported.
    ...
    # 4) Specific family fallback
    wanted_family = "round1&2&3&4"
    family = [
        s for s in sheets
        if _sheet_base_key(s).startswith(wanted_family)
    ]
    if len(family) == 1:
        print(
            f"[SheetCheck] Requested sheet was absent, but exactly one "
            f"R1-R4 sheet was found: {family[0]!r}"
        )
        return family[0]

    print("\n" + "=" * 88)
    print("[SheetError] 找到了 Excel 文件，但无法唯一确定 R1-R4 工作表。")
    print(f"[SheetError] 请求的名字: {req_text!r}")
    print("[SheetError] Excel 中真实存在的工作表如下：")
    for s in sheets:
        print("  -", repr(s))
    print("=" * 88)
    raise ValueError(
        "Could not uniquely resolve the R1-R4 worksheet. "
        "See the printed sheet names above."
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sheet-name", default=FORCED_SHEET_NAME, help="Training sheet; default is the requested Round1&2&3&4 sheet.")
...
def main() -> None:
    ...
    # Dedicated validation run: always use the exact normalized target.
    args.target_mode = "normalized"
    args.sheet_name = FORCED_SHEET_NAME

    print("=" * 88)
    print("[R1-R4 PUBLICATION MODELING RUN]")
    print(f"[WorkbookDefault] {FORCED_DATA_PATH}")
    print(f"[SheetWanted] {FORCED_SHEET_NAME}")
    print(f"[TargetWanted] {FORCED_NORMALIZED_TARGET}")
    print(f"[NormalizationVersion] {NORMALIZATION_VERSION}")
    print("[Purpose] Compare cumulative R1, R1+R2, R1+R2+R3, R1+R2+R3+R4.")
    print("=" * 88)
    ...
    output_dir = args.output_dir or os.path.join(
        ...
        "lnp_outputs",
        "R1R4_publication_nestedCV_" + pd.Timestamp.now().strftime("%Y%m%d_%H%M%S"),
    )
    ...
    training_sheet_requested = parse_sheet_argument(args.sheet_name)
    training_sheet = resolve_training_sheet_name(
        data_path, training_sheet_requested
    )
    print(f"[SheetUsed] {training_sheet!r}")
    original = pd.read_excel(data_path, sheet_name=training_sheet)

    # Fail immediately if the exact requested column is not present.
    exact_target_col = find_column(original, [FORCED_NORMALIZED_TARGET])
    if exact_target_col is None:
        raise ValueError(
            f"指定列 {FORCED_NORMALIZED_TARGET!r} 不存在。"
            f"\n当前 sheet 列名：{list(original.columns)}"
        )
    print(f"[ExactTargetPrecheck] Found: {exact_target_col!r}")
    ...
    # Target audit by cumulative round, before any model fitting.
    audit_rounds = sorted(
        [r for r in standardized["Round"].apply(normalize_round_label).unique()
         if r not in {"UNKNOWN", "", "nan", "None"}],
        key=round_sort_key,
    )
    print("\n[CumulativeTargetAudit]")
    for i in range(1, len(audit_rounds) + 1):
        inc = audit_rounds[:i]
        mask = standardized["Round"].apply(normalize_round_label).isin(inc).values
        yy = y.iloc[np.where(mask)[0]]
        print(
            f"  {'+'.join(inc):<15} n={len(yy):3d} "
            f"mean={yy.mean():.6f} SD={yy.std(ddof=1):.6f} "
            f"min={yy.min():.6f} max={yy.max():.6f}"
        )
    ...
    # Standard metrics, rather than a custom composite score, are used for reporting.
    summary = summary.sort_values(["R2", "Spearman"], ascending=[False, False]).reset_index(drop=True)
    ...
    if not args.skip_cumulative_round_analysis:
        ...
        if len(cumulative_summary):
            # Sort BEFORE dropping stage_order. The previous version selected a
            # compact set of columns first and then tried to sort by stage_order,
            # which caused KeyError: 'stage_order'.
            ensemble_cumulative = cumulative_summary[
                cumulative_summary["model"].eq("PrimaryTreeEnsemble")
            ].copy()

            if "stage_order" in ensemble_cumulative.columns:
                ensemble_cumulative = ensemble_cumulative.sort_values("stage_order")
            else:
                ensemble_cumulative = ensemble_cumulative.sort_values("stage")

            compact = ensemble_cumulative[
                ["stage", "n_samples", "n_formulation_groups",
                 "R2", "RMSE", "Spearman", "Top20_recall"]
            ].reset_index(drop=True)

            print("\n" + "=" * 88)
            print("[CUMULATIVE R1 -> R4 PRIMARY ENSEMBLE SUMMARY]")
            print(compact.to_string(index=False))
            print("=" * 88)

            compact.to_csv(
                os.path.join(output_dir, "CUMULATIVE_R1_R4_PRIMARY_ENSEMBLE.csv"),
                index=False,
                encoding="utf-8-sig",
            )
    ...
    run_info = pd.DataFrame([
        ...
        {"item": "normalization_version", "value": NORMALIZATION_VERSION},
